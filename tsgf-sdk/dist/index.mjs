...
class GameClient extends AWsClient {
    ...
    async startReconnect(currTryCount = 0, failReTry = true) {
        ...
        //如果是逻辑拒绝则不需要重连
        if (!this._playerToken || result.code == ErrorCodes.AuthReconnectionFail)
            failReTry = false;
...
})(ENetworkState || (ENetworkState = {}));

/**玩家输入帧类型*/
var EPlayerInputFrameType;
...
})(EPlayerInputFrameType || (EPlayerInputFrameType = {}));
...
})(ERoomCreateType || (ERoomCreateType = {}));
/**帧同步状态*/
var EFrameSyncState;
...
})(EFrameSyncState || (EFrameSyncState = {}));
/**私有房间的加入模式*/
var EPrivateRoomJoinMode;
...
})(EPrivateRoomJoinMode || (EPrivateRoomJoinMode = {}));

/**房间消息接收类型*/
var ERoomMsgRecvType;
...
})(ERoomMsgRecvType || (ERoomMsgRecvType = {}));
/*
let a: IRoomMsg = {
...
class Room {
    ...
    */
    ...
    async leaveRoom() {
        if (!this.__gameClient)
            return Result.buildErr('当前不在房间中!', ErrorCodes.RoomNotIn);
...
class GroupRoom {
    ...
    async backGroup() {
        if (!this._currGroupRoomId)
            return Result.buildErr('已经离开组队房间', ErrorCodes.RoomNotIn);
    ...
    async leaveGroup() {
        if (!this.currGroupRoom)
            return Result.buildErr('当前不在组队房间中!', ErrorCodes.RoomNotIn);
...
export { AHttpClient, AWsClient, EFrameSyncState, EMatchFromType, ENetworkState, EPlayerInputFrameType, EPrivateRoomJoinMode, ERoomCreateType, ERoomMsgRecvType, ErrorCodes, EventEmitter, EventHandlers, Game, GameClient, GroupRoom, GroupRoomEvents, HallClient, MatcherKeys, Result, Room, RoomEvents, arrCount, arrGroup, arrItemArrMerge, arrItemArrMergeConcat, arrRemoveItems, arrSkipAndLimit, arrSum, arrUpdateItems, arrWinner, delay, delayCanCancel, getGlobalSDKProvider, hasProperty, initSDK, initSDKProvider, numbersAdd, parseProcessArgv, parseProcessEnv };
